...
The system supports 7 game engines (Phaser 3, Three.js, Babylon.js, p5.js, Kaboom.js, PixiJS, and Python via Pyodide), an unlimited-length auto-continuation loop, and live integrations for physics intelligence, voice narration, on-chain export, and cross-session analytics.
...
When IBM watsonx Orchestrate is unavailable (network issue, quota, key not set), the system automatically falls back to Google Gemini 2.5 Flash — no user action required. Gemini receives the same detailed engine-specific system prompt and returns a complete HTML5 game. The status indicator on the Create page shows which AI generated the game.
...
| `/create` | Game builder — engine selector, Wolfram toggle, live pipeline animation, prompt input, play handoff |
| `/play` | Game runner — sandboxed iframe, render reload, code copy, HTML/ZIP export, fullscreen, ElevenLabs intro, Mint panel, Prediction Market panel |
...
- `↻ Render` to reload the current 2D or 3D game in the iframe
- `⧉ Code` to copy the generated source
- `⬇ HTML` to export the full runnable file
- `📦 ZIP` to export `index.html` plus a generated README
...
| `AUTH0_BASE_URL` | No | App base URL (`http://localhost:5000` or deployed URL) |
| `NEXT_PUBLIC_APP_URL` | No | Public app URL shown in health/config status |
...
# → http://localhost:5000
#
# If port 5000 is occupied on your machine:
PORT=5001 npm run dev
```

**Without IBM keys:** The app runs in demo mode — generates a full local game instantly with zero API calls. The "DEMO" badge appears top-left on the Create page.

## Operational Status

The app has been verified end-to-end with the current codebase:
- `npm run build` passes
- `npm run lint` passes
- Auth0 login redirects through `/api/auth/login`
- IBM watsonx Orchestrate agent listing and multi-pass generation work
- Gemini fallback generation works through `/api/gemini`
- Wolfram physics lookups work through `/api/wolfram`
- Snowflake analytics reads and writes work through `/api/analytics/query` and `/api/analytics/ingest`
- ElevenLabs voice discovery and audio synthesis work through `/api/voice`
- `/play` supports render reload, 2D/3D iframe playback, code copy, and HTML/ZIP export
...
src/
  app/
    api/
      ...
      voice/route.ts              ElevenLabs voice list + intro narration audio
    ...
    play/page.tsx                 Game runner — iframe, render, export, voice, mint, predict
...
README.md                         This file
.env.example                      All env vars with setup instructions + safe placeholders
