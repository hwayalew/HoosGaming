...
// Keep everything before line 62 (index 61) and after line 332 (index 332)
const before = lines.slice(0, 60).join("\n");   // lines 1-60
const after  = lines.slice(332).join("\n");       // lines 333+

// The CDN values are read from the file header so we embed them as literals
...
// Helper: escape backticks for embedding inside TypeScript template literals
function esc(s) { return s.replace(/`/g, "\\`").replace(/\$\{/g, "\\${"); }

// ─── Character hint extractor (TypeScript source) ───────────────────────────
const extractorSrc = `
...
// ─── Build the massive buildPrompt TypeScript function ───────────────────────
