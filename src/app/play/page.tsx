...
export default function PlayPage() {
  ...
  const exportZip = useCallback(async () => {
    ...
    try {
      ...
      const readmeMd = `# ${gameName}

> Generated by [Hoos Gaming](https://hoosgaming.replit.app) — IBM watsonx Orchestrate (89 AI agents)
...
Open \`index.html\` in any modern browser — no server, no install needed.
...
- Double-tap left/right for a dash
- Combine melee + shoot for higher combo multipliers
- Collect power-ups to unlock special abilities
...
${slug}/
  ...
  src/
    game.${ext}        ← Extracted game logic (no HTML wrapper)
  docs/
    HOW_TO_PLAY.md    ← Controls and mechanics
    UNITY_GUIDE.md    ← How to port this game to Unity
    EXTENDING.md      ← How to add features and modify the game
  README.md           ← This file
\`\`\`

## About the Generator

This game was built by 89 specialized AI agents (78 IBM watsonx Orchestrate + 11 virtual rendering) working in parallel:

- **NarrativeAgent** — story, lore, dialogue, objectives
- **CharacterRenderer** — 6-layer photorealistic character pipeline
- **AtmosphericRenderer** — smoke, fire, fog, weather particles
- **MaterialSimulator** — surface textures (metal/stone/concrete/fabric)
- **LightingEngine** — ambient, point lights, muzzle flash, bloom
- **WindPhysics** — Verlet cloth for capes, hair, banners
- **AnimationRigger** — walk/run/idle/aim/death cycles
- **EnvironmentPainter** — parallax sky, terrain, weather
- **ParticleSystem** — 300-particle pre-allocated pool
- **StyleAgent** — visual style adaptation (cartoon/photorealistic/pixel)

Built at HooHacks 2026 by students of UVA, UNC Pembroke, and Guilford College.
`;

      const howToPlay = `# How to Play — ${gameName}

## Objective

Survive, fight, and complete mission objectives. The game ends when you defeat the boss or run out of lives.
...
### Combat
- **Z** — Melee attack (close range, high damage)
- **X** — Shoot (ranged attack, costs ammo)
- **C** — Special ability / power dash
- **Shift** — Sprint (drains stamina)

### Game Systems
- **HP Bar** — Lose a life at 0 HP; game over with 0 lives
- **Stamina** — Drains on sprint; regens when idle
- **Combo Multiplier** — Chain kills for x2–x5 score multiplier
- **XP / Level** — Earn XP from kills; level up for stat boosts
- **Ammo** — Collect ammo pickups to restore ranged ammo

## Tips

1. Use dash to dodge boss projectiles
2. Hit enemies from above for bonus damage
3. Combo kills stack your score multiplier
4. Collect all pickups before entering the boss zone
5. Boss has 3 phases — watch for the telegraph before each attack
`;

      const unityGuide = `# Unity Adaptation Guide — ${gameName}

This guide explains how to re-implement this HTML5 game in Unity.

## Concept Mapping

| HTML5 / Canvas | Unity Equivalent |
|----------------|-----------------|
| \`<canvas>\` 2D context | Unity 2D with Sprite Renderers |
| \`requestAnimationFrame\` | \`MonoBehaviour.Update()\` |
| \`ctx.fillRect()\` | \`SpriteRenderer\` or \`Graphics\` component |
| \`createLinearGradient()\` | Shader Graph gradient node |
| \`ctx.shadowBlur\` | Unity Post-Processing Bloom effect |
| \`AudioContext.createOscillator()\` | \`AudioSource\` + procedural audio clips |
| Phaser physics arcade | Unity 2D Rigidbody + CircleCollider2D |
| Three.js WebGL | Unity Standard 3D scene |
| Babylon.js PBR materials | Unity URP Lit shader |
...
\`\`\`
Unity Hub → New Project → 2D (URP) or 3D (URP)
Package Manager → Install: Cinemachine, Input System, Post Processing
\`\`\`
...
public class PlayerController : MonoBehaviour {
    public float moveSpeed = 240f;   // maps to WALK_SPD constant
    public float runSpeed  = 400f;   // maps to RUN_SPD constant
    public float jumpForce = 580f;   // maps to JUMP_VEL constant
    public float gravity   = 580f;   // maps to GRAVITY constant (px/s²)
...
\`\`\`csharp
// EnemyAI.cs — maps to the 4-type enemy system
public enum EnemyType { Grunt, Ranger, Heavy, Aerial }
public class EnemyAI : MonoBehaviour {
    ...
    void Update() {
        switch (type) {
            case EnemyType.Grunt: PatrolAndCharge(); break;
            case EnemyType.Ranger: PatrolAndShoot(); break;
            case EnemyType.Heavy: SlowChargeArmored(); break;
            case EnemyType.Aerial: FlyAndDive(); break;
        }
    }
}
\`\`\`
...
\`\`\`
CharacterRenderer layers → Sprite layers + Shader Graph
AtmosphericRenderer smoke → Unity Particle System (Shuriken)
MaterialSimulator → URP material presets with textures
LightingEngine → Unity Light2D (point, spot, global)
WindPhysics cloth → Unity Cloth component or VFX Graph
ParticleSystem → Unity Particle System with sub-emitters
\`\`\`

### 5. Audio

\`\`\`csharp
// Maps Web Audio API procedural sounds to Unity clips
// sfx_shoot    → AudioSource.PlayOneShot(shootClip)
// sfx_jump     → AudioSource.PlayOneShot(jumpClip)
// bgm          → AudioSource.Play() with loop=true
\`\`\`

### 6. HUD

Create a Canvas (Screen Space — Overlay) with:
- Image (HP bar) — set via \`healthBar.fillAmount = hp / maxHp\`
- Image (Stamina bar)
- TextMeshPro (Score, Combo, Lives, Ammo)
- RawImage (Mini-map render texture)

## Physics Constants

The game uses these base values (may have been modified by Wolfram|Alpha):

| Constant | Value | Unity Setting |
|----------|-------|---------------|
| Gravity | ~580 px/s² | Rigidbody2D.gravityScale × Physics2D.gravity |
| Walk speed | 240 px/s | moveSpeed × sprite size / pixels-per-unit |
| Jump velocity | 580 px/s | jumpForce |
| Player HP | 100 | health field |
| Boss HP | 150–200 | bossHealth field |

## Recommended Unity Packages

- **Cinemachine** — camera follow + shakes (replaces camera.startFollow + shake)
- **DOTween** — tweening for UI animations (replaces manual lerp)
- **2D Extras** — tile rules (replaces platform array)
- **Universal RP** — PBR materials + post-processing
- **Input System** — rebindable controls
`;

      const extendingMd = `# Extending the Game — ${gameName}

## Adding a New Enemy Type

1. Locate the enemy drawing section in \`src/game.${ext}\`
2. Duplicate an existing enemy class / object
3. Change: hp, speed, attackRange, spriteDraw function, AI behavior
4. Add spawn logic in the level zone definitions

## Adding a New Weapon

1. Find the weapon constants near the top of the game code
2. Add a new entry: \`{ name, damage, range, cooldown, ammo }\`
3. Add draw code in the weapon render section
4. Wire to a key (e.g., key 3, 4) in the input handler

## Changing Physics

Search for \`GRAVITY\`, \`WALK_SPD\`, \`JUMP_VEL\` near the top of the file.
These can be tweaked directly. They may have been set by Wolfram|Alpha based
on your game's environment (moon, underwater, etc.).

## Adding Levels

Find the \`LEVEL_WIDTH\` or zone boundary constants. Extend the platform array
and enemy spawn definitions for each new zone.

## Changing Visual Style

Look for \`createLinearGradient\` calls in the character drawing code.
Change the color stops to shift the visual palette. For cartoon mode, 
replace gradient fills with flat \`fillStyle\` colors and add \`strokeStyle\` outlines.

## Triggering Character Voices

Call \`window.hoosSpeech(text, character, emotion)\` anywhere in the game:

\`\`\`javascript
hoosSpeech("Watch out — there are more!", "hero", "excited");
hoosSpeech("You dare challenge me?", "boss", "angry");
hoosSpeech("The ancient seal is broken.", "narrator", "neutral");
\`\`\`
...
`;
  ...
  "name": "${slug}",
  "version": "1.0.0",
  "description": "${gameName} — generated by Hoos Gaming (IBM watsonx Orchestrate)",
  "scripts": {
...
`;
      ...
      const zip = zipSync({
        ...
        [`src/game.${ext}`]:    strToU8(sourceContent),
        "README.md":            strToU8(readmeMd),
        "docs/HOW_TO_PLAY.md":  strToU8(howToPlay),
        "docs/UNITY_GUIDE.md":  strToU8(unityGuide),
        "docs/EXTENDING.md":    strToU8(extendingMd),
  ...
  if (!hasCode) {
    return (
      <div className="play-empty">
        ...
        <p className="play-empty-sub">Build a game first — 89 AI agents generate complete, playable code with sounds.</p>
