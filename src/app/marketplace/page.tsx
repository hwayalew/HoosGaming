  ...
  const mintCurrentGame = async () => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        setMintResult({ error: data.error ?? "Upload failed" });
      }
    } catch (e) {
    ...
    <div className="mkt-shell">
      ...
      <main className="mkt-main">
        <div className="an-header">
          <div className="sh-pre">IPFS · NFT.storage · Phantom (optional)</div>
          <h1 className="sh-h2" style={{ fontSize: 32 }}>Game Marketplace</h1>
          <p className="sh-p">
            Upload your generated HTML to IPFS via NFT.storage and track listings here. On-chain compressed NFT minting is not implemented in this repo — the API stores assets on IPFS only.
          </p>
        </div>

        {/* Mint panel */}
        <div className="mkt-mint-panel">
          <div className="mkt-mint-title">Upload your latest game to IPFS</div>
          <p style={{ color: "var(--muted)", fontSize: 12, fontFamily: "var(--mono)", marginBottom: 16, lineHeight: 1.6 }}>
            The server uploads your HTML and metadata to IPFS through NFT.storage only. Wallet address is recorded in metadata;
            this app does not submit a Solana or Metaplex mint transaction.
          </p>
              ...
              {mintResult.error ? (
                ...
                <div>
                  <div style={{ color: "#10b981", fontFamily: "var(--mono)", fontSize: 11, marginBottom: 6 }}>
                    ✓ Uploaded to IPFS · ID: {mintResult.gameId}
                  </div>
          ...
          {filtered.map(game => (
            <div key={game.gameId} className="mkt-card">
              ...
              <div className="mkt-card-body">
                ...
                <div style={{ marginTop: 6, fontFamily: "var(--mono)", fontSize: 9, color: "var(--muted)" }}>IPFS listing · {game.ts}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ textAlign: "center", padding: "40px 0", color: "var(--muted)", fontFamily: "var(--mono)", fontSize: 11 }}>
          Listings are HTML on IPFS (NFT.storage). Phantom is only needed to attach a wallet address to metadata.<br />
          <a href="https://phantom.app" target="_blank" rel="noreferrer" style={{ color: "var(--c1)" }}>Install Phantom →</a>
        </div>
      </main>
    </div>
  );
